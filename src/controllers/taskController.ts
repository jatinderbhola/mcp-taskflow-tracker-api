...
export class TaskController {
   ...
   */
  static getTasksByProjectId = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
   ...
   */
  static getWorkloadAnalysis = async (
    req: Request,
    res: Response,
    next: NextFunction,
  ): Promise<void> => {
    try {
      ...
      const workloadScore = Math.min(
        100,
        Math.max(0, totalTasks * 10 + overdueTasks * 20 + statusBreakdown.IN_PROGRESS * 15),
      );

      const insights = [
        `${assignee} has ${totalTasks} total tasks`,
        overdueTasks > 0
          ? `⚠️ ${overdueTasks} overdue tasks need attention`
          : '✅ No overdue tasks',
        statusBreakdown.IN_PROGRESS > 0
          ? `🔄 ${statusBreakdown.IN_PROGRESS} tasks in progress`
          : 'No active tasks',
      ];

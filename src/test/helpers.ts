...
import { serializeProject, serializeTask, SerializedProject, SerializedTask } from '../utils/dateUtils';

/**
 * Create a test project
 */
export async function createTestProject(override: Partial<Project> = {}): Promise<SerializedProject> {
 ...
 * Create a test task
 */
export async function createTestTask(projectId: string, override: Partial<Task> = {}): Promise<SerializedTask> {
...
export const api = {
    ...
    projects: {
        ...
        update: (id: string, data: Partial<Project>) => request(app).put(`/api/projects/${id}`).send(data),

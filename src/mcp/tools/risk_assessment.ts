...
export class RiskAssessmentProcessor {
  ...
  private async enhanceRiskAssessment(baseAssessment: any): Promise<EnhancedRiskAssessment> {
    ...
    enhanced.insights = [...baseAssessment.insights, ...this.generatePatternInsights(enhanced)];
  ...
  private calculateRiskBreakdown(assessment: any): any {
    ...
    breakdown.dependencies = Math.min(
      100,
      (assessment.blockedTasks + assessment.overdueTasks) * 10,
    );
  ...
  private generateEarlyWarnings(assessment: EnhancedRiskAssessment): string[] {
    ...
    const criticalPatterns = assessment.patterns.filter(
  ...
  private generatePatternInsights(assessment: EnhancedRiskAssessment): string[] {
    ...
    if (assessment.patterns.length > 0) {
      ...
      const highSeverityPatterns = assessment.patterns.filter(
        p => p.severity === 'HIGH' || p.severity === 'CRITICAL',
      );
      if (highSeverityPatterns.length > 0) {
        insights.push(
          `${highSeverityPatterns.length} high-severity patterns need immediate action`,
        );
      }
    }
    ...
    if (assessment.trend.direction !== 'STABLE') {
      insights.push(
        `Risk trend is ${assessment.trend.direction.toLowerCase()} with velocity ${assessment.trend.velocity}`,
      );
    }

    // BREAKDOWN INSIGHTS
    const highestRisk = Object.entries(assessment.riskBreakdown).sort(([, a], [, b]) => b - a)[0];
   ...
   */
  private async generateProjectComparison(
    ...
    try {
      ...
      return {
        ...
        performanceRelative: this.determineRelativePerformance(
          assessment.riskScore,
          projectBenchmarks.averageRisk,
        ),
      ...
      return {
        ...
        performanceRelative:
          assessment.riskScore < 50 ? 'BETTER' : assessment.riskScore > 50 ? 'WORSE' : 'SIMILAR',
        lessonsLearned: [
    ...
    try {
      ...
      const averageRisk =
        projectRisks.length > 0
          ? Math.round(
              projectRisks.reduce((sum, project) => sum + project.riskScore, 0) /
                projectRisks.length,
            )
          : 50;
      ...
      if (projectRisks.some(p => p.riskScore < 30)) {
        lessonsLearned.push(
          'Low-risk projects maintain regular task completion and minimal blockers',
        );
      }

      if (projectRisks.some(p => p.riskScore > 70)) {
        lessonsLearned.push(
          'High-risk projects often have accumulating overdue tasks requiring intervention',
        );
      }

      if (projectRisks.filter(p => p.status === 'COMPLETED').length > 0) {
        lessonsLearned.push(
          'Successful projects demonstrate consistent progress tracking and proactive issue resolution',
        );
      }
   ...
   */
  private determineRelativePerformance(
    currentRisk: number,
    benchmarkRisk: number,

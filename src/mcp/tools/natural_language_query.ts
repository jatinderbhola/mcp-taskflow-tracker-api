...
export class NaturalLanguageQueryProcessor {
            ...
            switch (parsedQuery.intent) {
                ...
                case 'analyze_workload':
                    if (parsedQuery.filters.assigneeName) {
                        const workloadAnalysis = await this.apiClient.getWorkloadAnalysis(parsedQuery.filters.assigneeName);
                ...
                case 'assess_risk':
                    if (parsedQuery.filters.projectId) {
                        const riskAssessment = await this.apiClient.getRiskAssessment(parsedQuery.filters.projectId);
    ...
    private generateTaskRecommendations(data: any[]): string[] {
        ...
        if (data.length === 0) {
            recommendations.push('Consider adjusting search criteria or checking if tasks exist for the specified filters');
...
export const naturalLanguageQueryTool = {
    ...
    parameters: z.object({
        prompt: z.string()
            ...
            .describe('Natural language query (e.g., "Show me Sarah\'s overdue tasks", "Analyze project health")')
    }),

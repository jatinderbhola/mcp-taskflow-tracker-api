...
export class EntityDiscovery {
    ...
    async discoverEntities(prompt: string): Promise<DiscoveredEntities> {
        ...
        try {
            ...
            const unknownEntities = this.extractUnknownEntities(lowerPrompt, peopleMatches, projectMatches);
            const suggestions = await this.generateSuggestions(unknownEntities, peopleMatches, projectMatches);
            ...
            return {
                ...
                projects: [],
                unknownEntities: [],
                suggestions: ['Unable to process query. Please try rephrasing or check network connectivity.']
            };
        }
    }
    ...
    private async generateSuggestions(
        ...
        if (unknownEntities.length > 0) {
            suggestions.push(`I don't recognize: ${unknownEntities.join(', ')}. Please check the spelling.`);
            ...
            if (unknownEntities.some(entity => /^[A-Z][a-z]+$/.test(entity))) {
                try {
                    ...
                    if (allPeople && allPeople.length > 0) {
                        suggestions.push(`Available people: ${allPeople.slice(0, 5).join(', ')}${allPeople.length > 5 ? '...' : ''}`);
                    }
                } catch (error) {
